...
public class DataHandler {
	...
	public Subject loadSubject(int subnum){
		...
		sub.percentage=(int) SubjectAdapter.getPer(sub.attended,sub.conducted);
		//Round off to greater value
		if (SubjectAdapter.getPer(sub.attended,sub.conducted) > sub.percentage)
			sub.percentage += 1;
			
		sub.regdate=str.get(breaker+7);
		sub.classnbr=str.get(breaker+8);
		
		//REMOVE THIS TO GET DETAILS IN CHENNAI CAMPUS
		if (preferences.getBoolean("ISVELLORE", true))
